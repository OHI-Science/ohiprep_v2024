...
## Download copernicusmarine tool in python
...
def install(package): # a function to download the copernicusmarine package
    subprocess.check_call([sys.executable, "-m", "pip", "install", package]) # `sys.executable` is the path to the python executable. for example, "/usr/bin/python3" 
...
```{python}
#| eval: false
!pip install copernicusmarine # to not run every time, but show how to install copernicusmarine package via pip if needed

import os
import copernicusmarine

# set the credentials made on the CMS website
copernicusmarine.configure(username='slecuona', password='OceanHealth2024') # ERROR: need to work on this

# define cms_dir, since the python chunk cannot necessarily read objects from previous R chunks/in the environment
...
print(f"Data downloaded to: {os.path.join(cms_dir, 'carbon_data.nc')}") # print in console: data download finished!
```
