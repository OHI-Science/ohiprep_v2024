...
The rest of the code in the script does not use the manually downloaded data, but if it is necessary you can read it in using this method and proceed. However, keep in mind that automation is more reproducible.
...
# class       : SpatRaster 
# dimensions  : 713, 1440, 13  (nrow, ncol, nlyr)
# resolution  : 0.25, 0.25  (x, y)
# extent      : -180, 180, -88.25, 90  (xmin, xmax, ymin, ymax)
# coord. ref. : lon/lat WGS 84 (EPSG:4326) 
# sources     : oa_rescaled_2010.tif  
#               oa_rescaled_2011.tif  
#               oa_rescaled_2012.tif  
#               ... and 10 more source(s)
# names       : oa_an~_2010, oa_an~_2011, oa_an~_2012, oa_an~_2013, oa_an~_2014, oa_an~_2015, ... 
...
# toc() 
annual_avg_rescaled_moll_dir <- here::here(oa_dir, version_year, "int","annual_avg_rescaled_moll") 
...
terraOptions(threads = 4) # to set number of cores

tic()
for (i in seq_along(oa_rescaled_annual_files)){
  
  yr = as.numeric(substr(oa_rescaled_annual_files, nchar(oa_rescaled_annual_files)-7, nchar(oa_rescaled_annual_files)-4)) # get year of file using the file's path/name
  ...
  r <- terra::rast(oa_rescaled_annual_files[i])
  
  # reprojecting to the OHI zones file to ensure they have the same extent and proj
  r_moll <- terra::project(r, zones)
  
  # resample to ensure they have the same resolution
  r_moll_resample <- terra::resample(x = r_moll, y = zones)
  
  ## write raster to int folder in prs_oa
  writeRaster(r_moll_resample, 
              filename = paste0(annual_avg_rescaled_moll_dir, '/oa_rescaled_moll_', yr, ".tif"),
              overwrite = TRUE)
}
toc()

# Error: [writeRaster] the number of filenames should either be one, or equal to the number of layers
# In addition: There were 16 warnings (use warnings() to see them)

# ... 15: In x@ptr$warp(y@ptr, "", method, mask[1], align_only[1],  ... :
#   GDAL Error 1: Too many points (529 out of 529) failed to transform, unable to compute output bounds.
# 16: In x@ptr$warp(y@ptr, "", method, mask[1], align_only[1],  ... :
#   GDAL Message 1: Unable to compute source region for output window 33783,2590,4827,2591, skipping.
```
...
# ---- reproject the historical raster -----
