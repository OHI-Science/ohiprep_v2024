...
fofm_tonnes <- read_csv(here(current_np_dir, "int", "mean_catch_FOFM.csv")) %>%
  ...
  ungroup() %>%
  group_by(rgn_id, product) %>%
  summarise(tonnes = mean(tonnes)) %>% ##calculate 5 year mean of catch -- note: in v2024, there were only 2 years in the former year column: 2018 and 2019 -- could be due to data year cap of 2019 in SAUP (downloaded in 2022 with data up to year 2019, hasn't been updated since)
  ungroup() %>%
  full_join(rgns_eez) %>%
...
orn_tonnes <- np_tonnes %>%
  dplyr::filter(year %in% years, # filter to 5 year range defined in the beginning of the first methods chunk
                product == "ornamentals") %>% 
...
check_older_prep <- check_older %>% 
  ...
  select(rgn_id, rgn_name, product, previous_diff, weight_2023, weight_2022) %>% 
...
combined_check %>% arrange(desc(abs(diff)), desc(abs(previous_diff))) %>% 
  head(n = 10)

combined_check %>% arrange(desc(abs(two_yr_diff))) %>% head(n = 10)
```
