...
np_harvest_orn <- readr::read_csv(here(current_np_dir, "int", "np_harvest_ornamentals_tonnes.csv"))

# join sustainability and tonnes
sust_tonnes <- np_harvest_orn %>% 
  full_join(orn_sust, by = join_by("rgn_id", "year", "product"))


# calculate sustainable harvest
orn_sust_harvest <- sust_tonnes %>% 
  mutate(sust_harvest = tonnes * sustainability)


# check
summary(orn_sust_harvest)
...
year_max <- max(orn_sust_harvest$year)

sust_tonnes_rolling <- orn_sust_harvest %>%
...
sust_tonnes_rolling <- orn_sust_harvest %>%
  ...
  dplyr::select(rgn_id, rgn_name, product, year, tonnes, sust_tonnes, sust_tonnes_orig)
```
