...
-   `mar_split()` function in `mar_fxs.R` was updated to use pivot_longer instead of gather, and the new grouped name of "Bonaire, Sint Eustatius and Saba" was accounted for

# 2023 updates from 2022 assessment

-   New year of FAO mariculture yield data for 2021. Downloaded the data online through Fishstat online query instead of using the fishstatj software. Updated instructions for downloading the data. Updated the script to work with minor changes in formatting in the new data.
...
```{r fao-data-prep}
# v2024 update:
# source 2024 FAO query data tidying function
...
mar_sp <- read.csv(here::here(current_mar_dir, "raw", "species_list.csv"), stringsAsFactors=FALSE) %>%
...
mar_value_filter <- mar_join_int %>%
  ...
  dplyr::ungroup() # remove groupigng
...
## Serbia and Montenegro: separate data for Montenegro beginning in 2006; Serbia is landlocked; 
...
maric <- read_csv(here(current_mar_dir, "output", "MAR_FP_data.csv"))
