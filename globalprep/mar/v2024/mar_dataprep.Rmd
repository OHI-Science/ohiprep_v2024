...
-   fixed issues system-dependent file paths using `{here}` package

-   made file names reproducible with updated entries in setup chunks

-   updated formatting of code and comments

-   rewrote data cleaning to follow tidyverse style more closely and be more reproducible & less prone to human error 

-   updated `mar_split()` function in `mar_fxs.R` to use `pivot_longer()` instead of `gather()`, and the new grouped name of "Bonaire, Sint Eustatius and Saba" was accounted for

-   fixed an issue in `mar_fxs.R` where updated data was not being properly bound to original data

-   renamed variables so that iterations of data processing steps weren't overwriting the same variable -- allows for comparison between different stages of dataprep
...
**Reference**:\
<https://www.fao.org/fishery/en/collection/aquaculture?lang=en> Release date: March 2023 FAO Global aquaculture production Quantity (1950 - 2021) Global aquaculture production (Value) (1950-2022)
...
source(here::here("globalprep", "mar", version_year, "mar_fxs.R")) # functions specific to mariculture dealing with compound countries
source(here::here("workflow", "R", "fao_fxn.R")) # old function for cleaning fao data
#source(here::here("workflow", "R", "fao_online_portal_clean.R")) #new fao cleaning function # switched to new version in 2024
...
mar_raw <- read_csv(file.path(FAO_mariculture_dir, current_data_folder,
...
mar_rgn_yrs_filled <- pivot_longer(mar_rgn_spread,
...
## 8200 of these out of 35023 + 8200 (43223) cases had NA values converted to 0 - 2024 assessment = 0.1897 ≈ 19% of values
```
...
write.csv(maric, file = here::here(current_mar_dir, "output", "MAR_FP_data.csv"),
          row.names = FALSE)
```
...
sw_sus <- read.csv(file.path(seafood_watch_dir, current_data_folder,
...
# v2024: potential rewrite:
# sw_sus_raw <- read_csv(file.path(seafood_watch_dir, current_data_folder, "SFW_Aquaculture_ratings_XXXXXX.csv"),
#                        na = c("NA", ""))
...
maric <- read_csv(here::here(current_mar_dir, "output", "MAR_FP_data.csv"))
...
v2024 note to self [COME BACK]: could be good to rename variables instead of continuously rewriting "maric" -- takes up more space in the environment but feels like better code hygeine
...
# _____ NA values out of ___ total obs; ~ __ without scores (2024)
```
...
write.csv(genEscapes_gf, here(current_mar_dir, "output", "GenEsc_gf.csv"), row.names = FALSE)
...
write.csv(data, here(current_mar_dir, "output", "GenEsc.csv"), row.names=FALSE)  
...
old <- read.csv(here(current_mar_dir, "output", "GenEsc.csv")) %>%
  filter(year == previous_latest_data_year) %>% 
  select(rgn_id, year, prs_score_old=pressure_score)

new <- read.csv(here(current_mar_dir, "output", "GenEsc.csv")) %>%
  filter(year == latest_data_year) %>%
