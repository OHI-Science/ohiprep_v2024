...
pkg <- c("raster", "fasterize", "sf", "sp", "rgdal", "fields", "here", "tictoc", "dplyr", "ggplot2", "foreach") # "fields" for colors in Status_Trend.R
...
# source("../../../workflow/R/common.R") # directory locations - this directory was copied into ohiprep_v2024 so modified to this directory's workflow
```
...
## This chunk is the method for acquiring the data through R. As the full 2023 data is not currently (06/25/2024) available for access, this might need to be updated when the data becomes available so that the comparisons can be made. Currently leaving it in because I (Dustin) will be attempting to update this method once the data becomes available to facilitate the process for the next OHI fellows. 
...
## UPDATE THESE!
assessYear <- "v2024" # change to reflect assessment year
previous_yr <- "v2023" # previous assessment year
final.year <- 2023 # final year of data (all months)
...
## need to make new file in the mazu filezilla 
  # command is 'mkdir + name of new folder' in terminal
  # EX: if making new file called 'v2024', type: 
  # "mkdir v2024"
...
# Change the date here to end on the last date that the date is available (per the NSIDC data access tool)
datestring <- '[(1978-11-01T00:00:00Z):1:(2023-09-30T00:00:00Z)]'
...
dim(df)
length(names(df)) == (2023 - 1978) * 12 + 2
# names(df)
# plot(df)
...
## Potential for the need to modify this if there are not data available for all 12 months of the most recent year. For example, this year (2024), there are only data up to september (month 09) rather than to december (month 12)
poles = c("n","s")
...
**This should be updated to account for already having read in the data and receiving an error or something else. Its re-reading in all data from mazu whereas there should be a "backup" function that says if the ...rgns_pts.x files already exist to just stack them to create the .rdata**
...
## WRap within evalWithTimeout
source("globalprep/hab_seaice/v2024/ObtainingData_Parallel.R") ## This has changed (2024) to account for the current working directory being ohiprep_v2024 rather than 'hab_seaice/v2024'
toc()
...
## Because the last full year of data is 2022, 'previous_year' reflects 2022 instead of 2023. Changing the variable to that of previous assessment rather than previous year of full data. This wont change anything for 2024 because 2023 uses 2022 data year (last full data year) instead of 2023 data.
...
comp_plot <- ggplot() + geom_point(data = ice_health_eez, aes(x = health_prev_assess, y = health)) + labs(x = "previous health (2012-2021)", y = "health (2012-2022)") + geom_abline(slope = 1, intercept = 0, col = "red")
...
## Changing current year back to 2022 because that's the last full year of data.
current_year = 2022

health_2022 <- health %>%
  dplyr::filter(year == current_year)

## Changing last year to 2021 because that's the 'last year of data' provided that the current year is using 2022 as its data 
last_year = 2021
...
year_trend_change_plot <-ggplot() + geom_point(data = trend_old_new, aes(x = trend_prev_assess, y = trend, text = paste("Region ID:", rgn_id, "<br>Habitat:", habitat))) + labs(x = "previous data year trend (2021)", y = "current data year trend (2022)") + geom_abline(slope = 1, intercept = 0, col = "red")
