...
old_not_in_new <- spp_maps2[!spp_maps2$shp_iucn_sid %in% spp_maps1$shp_iucn_sid, ]
...
fxn_test_results <- fix_this(fxn_ids) # according to the math, 50 items took about 20 seconds to run, wich = 0.4s/file*12240 files = 4896s/60s/m = 81.6m / 60m/h = 1.36 hrs
## Started at 5:59pm
## Finish(or error) at: 6:48
...
maps_already_rasterized <- list.files(file.path("/home/shares/ohi/git-annex/globalprep/spp/v2022/spp_risk_dists", 'spp_rasters'),
                                        pattern = '.csv') %>%
...
# results_na$iucn_sid is the vector
rewrite_bli_raster_function <- function(vector) {
  blis_to_rewrite <- vector
  for(i in seq_along(blis_to_rewrite)) {
    x <- blis_to_rewrite[i] # obtain iucn_sid
    # Identify file 
    spp_file_in <- file.path("/home/shares/ohi/git-annex/globalprep/spp/v2022/spp_risk_dists", 'spp_rasters',
                         sprintf('iucn_sid_%s.csv', x))
    if(!file.exists(spp_file_in)) {
      cat("Iucn sid", x, "not found in 2022 folder", "\n")
    } else {
...
rewrite_bli_raster_function(results_na$iucn_sid)
