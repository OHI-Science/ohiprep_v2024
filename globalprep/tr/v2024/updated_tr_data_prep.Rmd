...
dom_acc_guests_coef <- dom_acc_guests_agg %>% 
  ...
  mutate(
    total_guests_intercept = lm(total_guests ~ hotel_guests)$coefficient[1],
  ...
  mutate(
    total_guests_intercept_r2 = lm(total_guests ~ hotel_guests)$coefficient[1], # calc int and slope for grouping
    total_guests_slope_r2 = lm(total_guests ~ hotel_guests)$coefficient[2]
  ) %>% 
  ungroup() %>% 
  group_by(r1) %>% 
  mutate(
    total_guests_intercept_r1 = lm(total_guests ~ hotel_guests)$coefficient[1],
    total_guests_slope_r1 = lm(total_guests ~ hotel_guests)$coefficient[2]
  ) %>% 
  ungroup() %>% 
  distinct(rgn_id, r2, r1, total_guests_intercept, total_guests_slope, total_guests_intercept_r2, total_guests_slope_r2, total_guests_intercept_r1, total_guests_slope_r1) %>% # prep for left join, otherwise many-to-many
  select(rgn_id, r2, r1, total_guests_intercept, total_guests_slope, total_guests_intercept_r2, total_guests_slope_r2, total_guests_intercept_r1, total_guests_slope_r1) # select the columns that will be needed
            

# now use the georegions to find the intercept and slope accordingly
# dom_acc_guests_coef_gf <- georegions %>% 
#   left_join(dom_acc_guests_coef, by = c("rgn_id","r2","r1")) %>% 
#   group_by(r2) %>% # get values for r1 and r2, so that if regional is available, use that... then if r2 is available, use that... finally, use r1 if you can't otherwise.
#   mutate(
#     total_guests_intercept_r2 = mean(total_guests_intercept, na.rm = TRUE), # calc mean for grouping
#     total_guests_intercept_r2 = ifelse(is.nan(total_guests_intercept_r2), NA, total_guests_intercept_r2), # any NaN values to NAs
#     total_guests_slope_r2 = mean(total_guests_slope, na.rm = TRUE),
#     total_guests_slope_r2 = ifelse(is.nan(total_guests_slope_r2), NA, total_guests_slope_r2) # any NaN values to NAs
#   ) %>% 
#   ungroup() %>% 
#   group_by(r1) %>% 
#   mutate(
#     total_guests_intercept_r1 = mean(total_guests_intercept, na.rm = TRUE),
#     total_guests_intercept_r1 = ifelse(is.nan(total_guests_intercept_r1), NA, total_guests_intercept_r1), # any NaN values to NAs
#     total_guests_slope_r1 = mean(total_guests_slope, na.rm = TRUE),
#     total_guests_slope_r1 = ifelse(is.nan(total_guests_slope_r1), NA, total_guests_slope_r1) # any NaN values to NAs
#   ) %>% 
#   ungroup() %>% 
#   select(rgn_id, total_guests_intercept, total_guests_slope, total_guests_intercept_r2, total_guests_slope_r2, total_guests_intercept_r1, total_guests_slope_r1) # now we have intercept and slope values for all regions, lets use that to gapfill total_guests with hotel_overnights!
```
...
dom_acc_guests_gf1 <- dom_acc_guests_agg %>%
  ...
  mutate(
    total_guests_intercept_r2mean = mean(total_guests_intercept_r2, na.rm = TRUE), # calc mean for grouping
    total_guests_intercept_r2mean = ifelse(is.nan(total_guests_intercept_r2mean), NA, total_guests_intercept_r2mean), # any NaN values to NAs
  ...
  group_by(r1) %>%
  mutate(
    total_guests_intercept_r1mean = mean(total_guests_intercept_r1, na.rm = TRUE),
    total_guests_intercept_r1mean = ifelse(is.nan(total_guests_intercept_r1mean), NA, total_guests_intercept_r1mean), # any NaN values to NAs
  ...
  mutate(
    ...
    tourism_arrivals_ct = case_when(
      ...
      is.na(total_guests) & !is.na(hotel_guests) & !is.na(total_guests_intercept) ~ (hotel_guests*total_guests_slope) + total_guests_intercept,
      is.na(total_guests) & !is.na(hotel_guests) & is.na(total_guests_intercept) & !is.na(total_guests_intercept_r2mean) ~ (hotel_guests*total_guests_slope_r2mean) + total_guests_intercept_r2mean, # if regional lm coefficients weren't possible, go to r2 which is second least specific, then r1
      is.na(total_guests) & !is.na(hotel_guests) & is.na(total_guests_intercept) & is.na(total_guests_intercept_r2mean) & !is.na(total_guests_intercept_r1mean) ~ (hotel_guests*total_guests_slope_r1mean) + total_guests_intercept_r1mean,
      TRUE ~ tourism_arrivals_ct
    ),
    arrivals_gapfilled = ifelse(arrivals_method == "UNWTO - Hotel:Guests", "gapfilled", NA)
  ) %>% # example: 	South Korea 2017.  The numbers make sense because the estimated total_guests were larger than hotel_guests (makes sense because total includes other establishments than just hotels) and we can compare to hotel_overnights, of which tourism_arrivals_ct is about half ish, which based on our knowledge of mean_stays (found later on in raw gf for dom) the average is between 2-3 nights.  So this makes sense!
  # same with Algeria and China. To check, run code without the last select line
...
After I implemented the coefficients by georegion instead of just having the georegion intercept and slope be the mean of the regional coefficients in each grouping, those above examples above did not make sense.  However, the whole reason why I chose to go that route instead of just taking the mean by the grouped area is because some small regions were having very weird guest sizes...much larger than expected.  For example, New Caledonia (rgn 5).  `hotel_guests` for 2014 are 147, but the estimated total_guest amount (`tourism_arrivals_ct`) using the r1 intercept and slope is now 2063.736. This is super large, as those values are in thousands.  When looking it up, it seems that  ["New Caledonia had 107,200 tourists"](https://www.ieom.fr/IMG/pdf/ne154_portrait_synthese_2014_nouvelle-caledonie_version_anglaise.pdf) in 2014. This means that the estimation was really large in comparison.  I am thinking that perhaps it makes more sense to force the intercept to be 0 (like by using lm(total_guests ~ 0 + hotel_guests)), and then only use the slope to estimate the total_guests from hotel_guests.  That way, we also don't have any negative total_guests values (which is currently happening to Togo).
...
length(unique(dom_acc_guests_gf1_noNA$rgn_name)) # we now have 65 unique countries, not amazing. Let's move on and see if we can gapfill further.
```
...
```{r}
# first, remove any regions that only have NAs
dom_gf4_coef <- dom_acc_guests_gf3 %>% 
  ...
  mutate(
    total_guests_intercept = lm(total_guests ~ hotel_overnights)$coefficient[1],
    total_guests_slope = lm(total_guests ~ hotel_overnights)$coefficient[2]
    ) %>% # obtain the intercept and slope for each region that has data
  ungroup() %>% 
  distinct(rgn_id, total_guests_intercept, total_guests_slope) %>% # prep for left join, otherwise many-to-many
  select(rgn_id, total_guests_intercept, total_guests_slope) # select the columns that will be needed
...
dom_gf4_coef_gf <- georegions %>% 
  left_join(dom_gf4_coef, by = "rgn_id") %>% 
  group_by(r2) %>% # get values for r1 and r2, so that if regional is available, use that... then if r2 is available, use that... finally, use r1 if you can't otherwise.
  mutate(
    total_guests_intercept_r2 = mean(total_guests_intercept, na.rm = TRUE), # calc mean for grouping
    total_guests_intercept_r2 = ifelse(is.nan(total_guests_intercept_r2), NA, total_guests_intercept_r2), # any NaN values to NAs
...
dom_acc_guests_gf4 <- dom_acc_guests_gf3 %>% 
  left_join(dom_gf4_coef_gf, by = "rgn_id") %>% 
  mutate(
    arrivals_method = ifelse(is.na(tourism_arrivals_ct) & !is.na(hotel_overnights), "UNWTO - Hotel:Overnights", arrivals_method), # do this first so that we can use when tourism_arrivals_ct is NA as a qualifier
    tourism_arrivals_ct = case_when(
      is.na(tourism_arrivals_ct) & !is.na(hotel_overnights) & !is.na(total_guests_intercept) ~ (hotel_overnights*total_guests_slope) + total_guests_intercept,
      is.na(tourism_arrivals_ct) & !is.na(hotel_overnights) & is.na(total_guests_intercept) & !is.na(total_guests_intercept_r2) ~ (hotel_overnights*total_guests_slope_r2) + total_guests_intercept_r2,
      is.na(tourism_arrivals_ct) & !is.na(hotel_overnights) & is.na(total_guests_intercept) & is.na(total_guests_intercept_r2) & !is.na(total_guests_intercept_r1) ~ (hotel_overnights*total_guests_slope_r1) + total_guests_intercept_r1,
      TRUE ~ tourism_arrivals_ct
    ),
    # Kenya is a great example to show that it worked!! same with Algeria and China. To check, run code without the last select line
...
summary(dom_acc_guests_gf4) # we now have 2688 out of 4617 observations.  This is now 58% filled!
 ...
 #     rgn_id        rgn_name             year           tourism_arrivals_ct
 # Min.   :  5.0   Length:4617        Length:4617        Min.   :  -8957    
 # 1st Qu.: 65.0   Class :character   Class :character   1st Qu.:   1372    
 # Median :123.0   Mode  :character   Mode  :character   Median :   6754    
 # Mean   :123.3                                         Mean   :  34629    
 # 3rd Qu.:184.0                                         3rd Qu.:  24094    
 # Max.   :250.0                                         Max.   :2321983    
 #                                                       NA's   :2688       
...
length(unique(arrivals_join$rgn_name)) # v2024: 161 regions that have data! This checks out!
```
...
length(unique(mean_domestic_guests$rgn_name)) # 162 regions??
setdiff(mean_domestic_guests$rgn_name, arrivals_aggregated$rgn_name) # there is one region that is named "NA"...Iran 191. go back and check on that
...
length(!is.na(arrivals_aggregated$tourism_arrivals_ct_int)) # 2254
length(!is.na(arrivals_aggregated$tourism_arrivals_ct_dom)) # 2254, yay! now we can aggregate international and domestic with the least amount of error with the data we have
