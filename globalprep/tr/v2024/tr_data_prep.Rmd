...
# -89074869  -9848813   -687175   1825167 154310314 
# 
# Coefficients:
#                         Estimate Std. Error t value Pr(>|t|)    
# (Intercept)            1.285e+05  1.403e+06   0.092    0.927    
# tourism_arrivals_trips 9.597e-01  1.955e-02  49.086   <2e-16 ***
# ---
# Signif. codes:  0 ‘***’ 0.001 ‘**’ 0.01 ‘*’ 0.05 ‘.’ 0.1 ‘ ’ 1
# 
# Residual standard error: 30900000 on 690 degrees of freedom
#   (5329 observations deleted due to missingness)
# Multiple R-squared:  0.7774,	Adjusted R-squared:  0.7771 
# F-statistic:  2409 on 1 and 690 DF,  p-value: < 2.2e-16
...
unwto_clean_names_dom <- name_2_rgn(df_in = join_dom_acc_trips, 
                                        fld_name = 'country',
                                        keep_fld_name = TRUE) %>%
  dplyr::select(rgn_id, rgn_name, year, tourism_arrivals_ct, tourism_arrivals_trips) # lost many land locked regions, duplicates were also found so we need to look into that after gapfilling
```


```{r}
# gapfilling the accommodations data using trips
...
arrivals_georegions_gf <- arrivals_georegions %>% 
  ...
  mutate( 
    tourism_arrivals_all = case_when(
      !is.na(lm_coef_r2) & is.na(tourism_arrivals_ct) ~ tourism_arrivals_ct*lm_coef_r2,
      is.na(lm_coef_r2) & !is.na(lm_coef_r1) ~ tourism_arrivals_ct*lm_coef_r1,
