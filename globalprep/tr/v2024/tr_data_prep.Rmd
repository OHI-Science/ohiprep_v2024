...
while(na_flag) {
  ...
  unwto_gapfill_lm_2019_tourism_dom <- unwto_gapfill_lm_2019_tourism_dom %>%
    ...
    mutate(
      tourism_arrivals_all = case_when(
        is.na(tourism_arrivals_all) & !is.na(dplyr::lag(tourism_arrivals_all)) ~ dplyr::lag(tourism_arrivals_all) + lm_coef_tourism, # if there is a previous value, but now current value, gapfill using the lag plus the lm_coef_tourism
