...
while(na_flags) {
  ...
  # Check if there are still NAs left in either column
  na_flags <- any(is.na(unwto_gapfill_lm_2019_tourism_dom$tourism_arrivals_all)) # great, the only ones left as NAs here are Guatemala and Myanmar, which each only had one original value and should be dropped.
} ######stopped here
...
# combine back with any regions that had all NAs
unwto_gapfill_lm_2019_tourism_all_dom <- unwto_gapfill_lm_2019_tourism_dom %>%
  dplyr::select(-lm_coef_tourism) %>%
...
write_csv(gdp_raw_gf, here(int_dir, "gdp_raw_gf.csv"))
...
write_csv(tr_sust_gf, here(int_dir, "tr_sustainability_gf.csv"))
...
write_csv(tr_sust, here(int_dir, "tr_sustainability.csv"))
```
