...
columns = c("rank","economy","score","rank_change","score_change","diff_avg","rank_2","economy_2","score_2","rank_change_2","score_change_2","diff_avg_2", "rank_3","economy_3","score_3","rank_change_3","score_change_3","diff_avg_3")
...
sr_scores <- sr_scores_table %>% 
  ...
  separate_wider_position(
    main_column, # the issue here is that there is not a common delimiter, and the three columns blur together
    widths = c(rank = 2, economy = 24, score = 4, rank_change = 3, score_change = 5, diff_avg = 19, 
               ...
               rank_3 = 9, economy_3 = 26, score_3 = 8, rank_change_3 = 1, score_change_3 = 1, diff_avg_3 = 1),
    names_sep = NULL,
  ...
  select(rank, economy, score,  rank_2, economy_2, score_2, rank_3, economy_3, score_3) %>% 
  mutate(
    economy = str_trim(economy, side = "both"),
    ...
    economy_3 = str_replace(economy_3, pattern = "Trinidad and Tobago 3.5", replacement = "Trinidad and Tobago"),
    score_3 = str_replace(score_3, pattern = "2 -5", replacement = "3.52")
    )



#   # mutate(score_3 = str_replace(score_3, pattern = " ", replacement = ""))
#   
#   # separate_wider_delim(main_column, delim = " ", names = columns, too_many = "debug")
#   # separate_wider_delim(main_column, " ", names = columns, too_many = "debug")
#   # separate(main_column, into = columns, sep = " {2,}", extra = "merge") %>%
#   # separate(main_column, into = columns, sep = " {1,}", extra = "merge")
#   # separate_longer_delim(score, delim = " ")
#   # separate_wider_delim(main_column, delim = " -", names_sep = "", too_few = "align_start", cols_remove = FALSE) %>% # closest
#   # separate_wider_delim(1:ncol(.), delim = " ", names_sep = "", too_few = "align_start")
#   # mutate(main_column = gsub("%.*", "", main_column)) %>% 
#   # mutate(main_column = gsub("-.*", "", main_column)) %>% 
#   # separate_wider_delim(main_column, delim = " ", names_sep = "", too_few = "align_start", cols_remove = FALSE)
# 
# # another option using regex instead of delimiters because there are too many variable delimiters
# sr_scores <- sr_scores_table %>%
#   separate_rows('.', sep = "\n") %>%
#   rename(main_column = ".") %>%
#   slice(16:55) %>%
#   separate(main_column, into = columns, sep = "\\s{2,}|\\s-\\s|%")
# 
# # do more preprocessing before using separate
# sr_scores_table <- ttdi_raw[[11]] %>%
#   str_replace_all("", " ") %>% 
#   str_replace_all("\\s{2,}", " ") %>%
#   str_replace_all("\\s{3,}", " ") %>% 
#   str_replace_all("\\s{4,}", " ") %>% 
...
#   rename(main_column = ".") %>%
#   slice(16:55) %>%
#   separate(main_column, into = columns, sep = " ")
# 
# #####
# # using readr
# library(readr)
# sr_scores <- read_table(ttdi_raw[[11]], col_names = columns) %>% 
#   slice(12:59)
...
# Changing names that are not re2cognized by ohicore
ttdi <- ttdi %>%
