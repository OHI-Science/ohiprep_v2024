...
check_international <- unwto_dupe_fix %>% 
  filter(!is.na(tourism_arrivals_ct))
length(unique(check_international$rgn_id)) # v2024: 167 regions present!
```
...
unwto_clean_names_dom <- name_2_rgn(df_in = join_dom_acc_trips, 
                                        fld_name = 'country',
                                        keep_fld_name = TRUE) %>%
  dplyr::select(rgn_id, rgn_name, year, tourism_arrivals_all, gf_method, arrivals_gf) # lost many land locked regions, duplicates were also found so we need to look into that
...
# this shows rows that have at least one duplicate, including both the first and subsequent occurrences of each duplicated row. The approach of looking from the front and the back ensures that you capture all instances of duplicated data, not just the second and subsequent occurrences (which is what just a duplicated() call would give you).
...
domestic_clean_names_agg <- unwto_clean_names_dom %>% 
  ...
  summarize(
    ...
    gf_method = first(gf_method, na_rm = TRUE),
