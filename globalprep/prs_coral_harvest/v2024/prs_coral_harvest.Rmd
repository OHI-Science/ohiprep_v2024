...
source(here::here("globalprep", "prs_coral_harvest", v_scen_year, "R", "np_fxn.R"))
```
...
for (f in files){  
  ...
  m <- m %>%
    ...
    dplyr::select(-c(trade, Unit)) %>% # eliminate 'trade' column
...
# [9] "Sudan"                 "Sudan (former)"        "Un. Sov. Soc. Rep."     - these are all fixed in the group by and summarise below  
  ...
  # v2024: removed yugoslavia sfr in both quality and value
  # ascension saint helena and tristan da cunha in value
  
  # had the same duplicates as v2023; all are fixed in the group by and summarize (aggregating groups by ohi rgn_id among other variables, which sums the "duplicates" above into appropriate rgn_id groups)
  ...
  m_rgn <- m_rgn %>%
    ...
    dplyr::summarize(value = sum_function(value)) %>%
    dplyr::ungroup()
