...
* **Data:** 
    ...
    * Also brought in OECD data from their online [OECD Data Explorer](https://data-explorer.oecd.org/vis?df[ds]=DisseminateFinalDMZ&df[id]=DSD_SOE%40DF_SOE&df[ag]=OECD.ENV.EPI&dq=.A....&pd=1995%2C2024&to[TIME_PERIOD]=false&vw=tb)
...
eco_mar_usd_raw <- rbind.data.frame(mar_rev_rgn_temp, mar_rev_china_temp) %>% 
  relocate(c(rgn_id, rgn_name), .before = country) %>% 
  ungroup() %>% 
  select(-c(country, sector, data_source, Country)) %>% 
  dplyr::rename(usd = value)

## Using priceR for each unique combination of year, rgn_id, and sector --> aggregate function maybe 
??priceR

prices <- eco_mar_usd_raw$usd

from_dates <- eco_mar_usd_raw$year

to_dates <- 2017

country = "US"

converted_eco_mar_usd <- adjust_for_inflation(price = prices, from_date = from_dates, country = country, to_date = to_dates)

eco_mar_usd_raw$converted_usd = converted_eco_mar_usd 

## Checking how many unique countries are within the revenue data - 147 (2024 Assessment)
# length(unique(eco_mar_usd_raw$rgn_id))



# write_csv(eco_mar_usd_raw, here::here(data_path, "int/eco_mar_usd_raw.csv"))
```
