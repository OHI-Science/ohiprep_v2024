...
Remember that China subset we made out of the Macao and Hong Kong data? Now we're going to join that with a subset of the global country-level GDP data to find the tourism value for these areas. We will then join this data frame with the China tourism value data we read in earlier (we'll need to clean it first) and aggregate the values to yield one value per year for China as a region.
...
# Since we already have the Tourism value for mainland China
```
...
nbsc_tidy <- nbsc_clean %>% 
  ...
  pivot_longer(cols = x2023:x2004, # update this depening on year range of newly downloaded data
...
- join china_gdp (china subset of GDP data) with macao_hk_subset (gdp proportion subset of Macao and Hong Kong), calculate value (will be in Constant 2015 US$)
...
- run `name_2_rgn` function on this, make necessary adjustments
  -  then join with `no_mc_hk_subset`, call it `no_china_gdp_join` or something similar
  
- calculate usd value for `no_china_gdp_join`. call this `no_china_value`

- rbind `china_value` with `no_china_value`. call this `tourism_value`

- prep `tourism_value` for export, then write script, rerun eco_usd_adj.Rmd
