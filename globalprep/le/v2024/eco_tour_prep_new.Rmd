...
years_df <- tibble(geo_area_name = gdp_prop_clean$geo_area_name) %>% 
  ...
  summarize(year = seq(min(gdp_prop_clean$year),
                       max(gdp_prop_clean$year)))

# ---- join with gdp prop data ----
gdp_prop_yrs <- left_join(years_df, gdp_prop_clean,
...
heatmap_df <- gdp_rgn_agg %>% 
  mutate(status = !is.na(gdp_prop_percent)) %>% 
  ungroup()
...
raw_gdp_pct_data_summary <- heatmap_df %>% 
  ...
  summarize(n_data_points = sum(status)) %>% 
  arrange((n_data_points)) # show number of data points that we have for that region across the full time range 

raw_gdp_pct_data_summary
...
start_year <- 2008
stop_year <- 2019 # previously "cutoff_year"
...
five_yr_subset <- heatmap_df %>% 
  filter(year %in% c(recent_years)) %>% 
  group_by(geo_area_name) %>% 
 # group_by(rgn_id, rgn_name) %>% 
  summarize(n_data_points = sum(status)) %>% 
  filter(n_data_points >= 1)
...
no_china_filter <- no_china_filter %>% 
  filter(!geo_area_name %in% c("Argentina"))
```
...
-  join `china_value` with `nc_tourism_value`. call this `tourism_value`
