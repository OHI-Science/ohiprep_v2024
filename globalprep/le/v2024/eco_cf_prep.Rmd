...
mean_catch_ci_agg <- mean_catch_2017 %>% 
  filter(rgn_id %in% c("228", "227")) %>% 
  filter(year %in% c(2014:2017)) %>% 
  group_by(rgn_id, year) %>% 
  dplyr::summarize(catch = sum(mean_catch, na.rm=TRUE)) %>%
  ungroup() %>%
  group_by(rgn_id) %>% 
  mutate(avg_catch = mean(catch)) %>% 
  pivot_wider(names_from = "rgn_id",
              values_from = "avg_catch") %>% 
  rename(jersey = as.character(227),
         guernsey = as.character(228))

mean_catch_bes_agg <- mean_catch_2017 %>% 
  filter(rgn_id %in% c("245","249","248")) %>% 
  filter(year %in% c(2014:2017)) %>% 
  group_by(rgn_id, year) %>% 
  dplyr::summarize(catch = sum(mean_catch, na.rm=TRUE)) %>%
  ungroup() %>%
  ...
  pivot_wider(names_from = "rgn_id",
              values_from = "avg_catch") %>% 
  rename(bonaire = as.character(245),
         sint_eustatius = as.character(249),
         saba = as.character(248))

prop_g <- (mean_catch_ci_agg$guernsey[5])/(mean_catch_ci_agg$guernsey[5]+(mean_catch_ci_agg$jersey[1]))

prop_j <- (mean_catch_ci_agg$jersey[1])/(mean_catch_ci_agg$jersey[1]+(mean_catch_ci_agg$guernsey[5]))

prop_b <- (mean_catch_bes_agg$bonaire[1])/((mean_catch_bes_agg$bonaire[1])+(mean_catch_bes_agg$sint_eustatius[9])+(mean_catch_bes_agg$saba[5]))

prop_e <- (mean_catch_bes_agg$sint_eustatius[9])/((mean_catch_bes_agg$bonaire[1])+(mean_catch_bes_agg$sint_eustatius[9])+(mean_catch_bes_agg$saba[5]))

prop_s <- (mean_catch_bes_agg$saba[5])/((mean_catch_bes_agg$bonaire[1])+(mean_catch_bes_agg$sint_eustatius[9])+(mean_catch_bes_agg$saba[5]))
```
