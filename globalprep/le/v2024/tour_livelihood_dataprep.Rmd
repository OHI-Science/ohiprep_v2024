...
# total number of people employed by the tourism sector, (not actually in thousands like the title claimed (remember this!)) from UNWTO
...
tourism_labor_join <- left_join(labor_regions, tourism_un_jobs_ohi, by = c("rgn_id", "rgn_name", "eez_iso3", "admin_country_name", "year")) 
```
...
tourism_job_proportion <- tourism_labor_join %>%
  ...
  select(rgn_id, rgn_name, eez_iso3, country, year, labor_force_total, tourism_jobs, tourism_proportion)
```
...
Save data to `int` folder in le/vCurrentYear
```{r}
tourism_jobs_int <- tourism_job_proportion %>% 
  mutate(sector = "tour", # add on sector
         data_source = "UNWTO") # and data source columns

# uncomment to save data to int folder
#write_csv(tourism_jobs_int, here(int_dir, "tour_jobs.csv"))
```
