...
# Table of number of regions & countries that we have data for per year

```{r}
job_countries_per_year <- jobs_data_final %>% 
  group_by(year) %>% 
  summarize(n_countries = length(unique(rgn_name)))

job_countries_per_year

library(gt)

job_countries_per_year %>% gt()
```
