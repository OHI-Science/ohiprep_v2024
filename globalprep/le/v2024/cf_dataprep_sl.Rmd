...
# 
# bonaire/s.eustatius/saba british indian ocean ter          channel islands     french southern terr 
#                        1                        1                        1                        1 
#              isle of man     netherlands antilles                other nei         saint barthélemy 
#                        1                        1                        1                        1 
# saint helena/asc./trist.    saint-martin (french)           yugoslavia sfr 
#                        1                        1                        1 
# 
# These data were removed for not being of the proper rgn_type (eez,ohi_region) or mismatching region names in the lookup tables:
#            tmp_type
...
length(unique(cf_revenue$country))

cf_revenue_fix <- cf_revenue_rgn %>%
  mutate(country = case_when(
    country=="Bonaire/S.Eustatius/Saba" ~ "", # how to disaggregate instead of aggregate?
    country=="British Indian Ocean Ter" ~ "British Indian Ocean Territory",
    country=="Channel Islands" ~ "", # would be disaggregated into Guernsey and Jersey
    country=="" ~ " ", 
    country=="" ~ " ",
    country=="  " ~ " ",
    country=="  " ~ " ",
...
# look at rasterized fishing effort, to determine the relative proportion of it within each eez, and distribute catcbh relative to that proportion
cf_revenue_rgn <- name_2_rgn(df_in = cf_revenue_fix, 
                             fld_name='country',
                             flds_unique = c("year"))

oecd_duplicates <- oecd_fisher_jobs_rgn[duplicated(oecd_fisher_jobs_rgn[, c("country", "year")]),]

oecd_ref_duplicates <- unique(oecd_duplicates$country)
oecd_rgn_duplicates <- unique(oecd_duplicates$rgn_name)
