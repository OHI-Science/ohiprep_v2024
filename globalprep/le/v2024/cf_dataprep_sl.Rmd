...
cf_wage_ppp_raw <- read_csv(here(oecd_dir, "OECD.SDD.NAD,DSD_NAMAIN10@DF_TABLE4,1.0+A.AUS+AUT+BEL+CAN+CHL+COL+CRI+CZE+DNK+EST+FIN+FRA+DEU+GRC+HUN+ISL+IRL+ISR+ITA+JPN+KOR+LVA+LTU+LUX+MEX+NLD+NZL+NOR+POL+PRT+SVK+SVN+ESP+SWE+CHE+TUR+GBR+USA...PPP_B1GQ........csv"))
...
Begin to create the proportion between tourism jobs and the total labor force
```{r}
...
length(unique(cf_job_iso$eez_iso3)) # 44
...
cf_job_labor_join_2 <- left_join(labor_force, cf_job_iso_agg, by = c("eez_iso3", "year")) %>% 
...
length(is.na(cf_job_labor_join_2$value))
```
