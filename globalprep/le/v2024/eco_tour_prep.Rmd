...
- figure out if population pct is of rural/urban or of total population
- check: add population counts for both designations for one country in 1 year, then divide rural count by total count, see if this equals rural pct or something else

```{r}
# check for grouping
# is_grouped_df(coastal_prop_tidy)

# make subset to test population values
prop_coast_test <- coastal_prop_tidy %>% 
  # select first country to test
  filter(countryname == "Aruba" & year == 1990)

# make rural and urban subsets
rural_test <- prop_coast_test %>% filter(designationname == "RURAL")
urban_test <- prop_coast_test %>% filter(designationname == "URBAN")

# extract values (for shorter math statements)
rural_pop_count <- rural_test$population_count
urban_pop_count <- urban_test$population_count

# find proportion of rural out of total coastal pop
rural_test_prop <- rural_pop_count / (rural_pop_count + urban_pop_count)

# see if rural proporiton is equal to pop pct value
rural_test$population_pct == round((rural_test_prop * 100), 0)

```

Okay! Now we know what this data is showing -- out of the total coastal population per country, what percent (and count) is Rural or Urban.

So, to make a coastal tourism multiplier, we need to aggregate the coastal population counts (rural + urban), then divide that coastal population count by the total population of the country (which we'll pull in from another dataset).


Overview of next steps

-   aggregate by countryname to get total coastal population count in 2010
-   run `ohicore::name_2_rgn`(subset out Macao & Hong Kong first)
-   bring in world population data 

    -   clean & tidy, select 2010 values
    -   run `ohicore::name_2_rgn`

-   join coastal population + total population in 2010
-   calculate coastal population proportion
-   join with appropriate tourism dataset & calculate appx. coastal tourism value 


#### Coastal population

Aggregate by `countryname` to get total coastal population count in 2010. We'll use this to calculate a fixed coastal population proportion multiplier to estimate coastal tourism value.

```{r}
# ---- create 2010 subset for fixed multiplier calculation ----
...
coastal_2010_pop <- coastal_2010_pop_subset %>% 
...
coastal_2010_pop %>% filter(country_name %in% c("United States of America"))
```

#### WorldBank: Total population

Global, country-level total population data (ranges from 1960-2023)

```{r WDI-test, eval=FALSE}
# tried using WDI package but got 502 Bad Gateway warning & timed out
# find code for World Bank total population dataset
wdi_datasets <- WDI::WDI_data$series

wb_pop_df_info <- wdi_datasets %>% 
  filter(name %in% c("Population, total"))

wb_pop_code <- wb_pop_df_info$indicator
# SP.POP.TOTL

WDI::WDI(country = "all", indicator = "SP.POP.TOTL", start = 2010, end = 2010)
```

```{r}
# read in total pop data from world bank
wb_total_pop_raw <- readr::read_csv(here(wb_dir, "WorldBank_global_country_population_1960-2023",
                                         "API_SP.POP.TOTL_DS2_en_csv_v2_1584446.csv"),
                                    skip = 4)
# note: values are total, not in thousands

# ---- cleaning & tidying ----
wb_total_pop <- wb_total_pop_raw %>% 
  # pivot years from wide to long
  tidyr::pivot_longer(
    cols = "1960":"2023", # update to whatver time frame makes sense
    names_to = "year",
    values_to = "total_pop"
  ) %>% 
  # clean column names
  janitor::clean_names() %>%  # running this after pivoting to reduce how much cleaning the year column needs
  # fix data types
  dplyr::mutate(year = as.numeric(year)) %>% 
  # select relevant columns
  dplyr::select(c(country_name, year, total_pop))

# make 2010 subset for calculating coastal pop proportion
wb_2010_pop <- wb_total_pop %>% 
  filter(year == 2010)
...
coastal_prop_pct <- coastal_pop_join %>% 
  mutate(coastal_prop = (coastal_pop / total_pop),
         coastal_pct = (coastal_prop * 100)) # proportion multiplier
...
coastal_tourism_join <- dplyr::full_join(coastal_value_clean, usa_coastal_tourism,
                                         by = join_by("rgn_id", "rgn_name", "year", "coastal_tour_value")) %>% 
  ungroup() %>% 
  mutate(rgn_id = as.numeric(rgn_id)) # prep for joining with OHI regions

# ---- join OHI regions with years df ----
rgn_yr_df <- tibble(rgn_name = region_names$rgn_name,
                       rgn_id = region_names$rgn_id) %>% # take the regions from region_names, even the ones we don't have data for
  group_by(rgn_id, rgn_name) %>% 
...
coastal_tourism_full <- dplyr::full_join(rgn_yr_df, coastal_tourism_join,
                                         by = join_by("rgn_id", "rgn_name", "year")) %>% 
  select(rgn_id, rgn_name, year, coastal_tour_value)
