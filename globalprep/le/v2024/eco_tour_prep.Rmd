...
# both are 1, so we can aggregate by renaming both Guadeloupe and Martinique to the combined region name

coastal_rgn %>% dplyr::filter(rgn_name %in% c("Northern Mariana Islands and Guam"))
# both are equal to 0 (the coastal pop dataset did not have data on these regions in 2010, but the coastal population percent was 100% for both regions in 1990 and 2000, so we will set these proportions equal to 1 in our next aggregation step by just assigning them the same name)

coastal_rgn %>% dplyr::filter(rgn_name %in% c("Puerto Rico and Virgin Islands of the United States"))
# great! for all of these, the proportion is 1 in both regions, so we can just assign them the same name
...
coastal_rgn_agg <- coastal_rgn %>% 
  dplyr::ungroup() %>% 
  dplyr::group_by(rgn_id, rgn_name, year) %>% 
  dplyr::summarize(coastal_prop_gf = case_when(
    # if it's one of the duplicate regions identified earlier, 
    rgn_name %in% c(coastal_dup_rgns) ~ 1, # fill coastal_prop_gf with a 1 (100% of pop within 100km of coast)
    # else, keep old value
    .default = coastal_prop_gf
  ))
