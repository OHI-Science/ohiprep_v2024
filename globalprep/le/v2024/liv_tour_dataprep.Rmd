...
COME BACK -- delete if not joining with OHI regions, also would need to address EEZ ISO3 issue by joining regions df by rgn_name, don't make iso3 column beforehand
...
labor_export <- labor_regions %>% 
  # adding "no data" values in the year column for countries that we have no data for 
  mutate(data_status = ifelse(is.na(year), "no_data", "yes_data"),
         admin_rgn_id = as.character(admin_rgn_id),
...
#write_csv(labor_export, here(int_dir, "labor_force.csv"))
```
...
un_job_data <- file.path(here(unwto_dir, "unwto-all-data-download_0.xlsx"))
...
tour_jobs_rgn <- name_2_rgn(df_in = un_jobs_cc, 
                       fld_name = 'country',
                       flds_unique = c("year"))
...
# Guadeloupe and Martinique: only have sparse data for Martinique, none for Guam -- can just sum

# aggregate: sum by rgn_name etc. ----
tour_jobs_rgn_agg <- tour_jobs_rgn %>%
  group_by(rgn_id, rgn_name, year) %>%
  dplyr::summarize(tourism_jobs_agg = sum(tourism_jobs, na.rm=TRUE) # causing issues...
            )%>%
  ungroup() %>% 
  filter(year >= 2009)
# COME BACK: make 0s into NAs?? 


# join with labor force data ------------
# COME BACK -- rejoin once labor force has been updated
# ISSUE -- tourism_jobs being populated with 0s instead of NAs.... super weird
