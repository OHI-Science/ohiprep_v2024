...
## Adding new species to species_list.csv ----
...
notes = c(NA,
          NA,
          "Not fully certain, but species distribution adds up.",
          NA,
          NA,
          NA,
          NA,
...
new_spp_add <- tibble(FAO_name = FAO_names, exclude = exclude, Taxon_code = Taxon_codes, family = families, notes = notes, notes_2 = notes2)

new_spp_add <- new_spp_add %>% 
  mutate(notes_2 = as.character(notes_2))

mar_sp_2 <- mar_sp %>% rbind(new_spp_add)

write.csv(mar_sp_2, here::here('globalprep/mar/v2023/raw/species_list.csv'))
...
```{r}
# For 2024: The countries that can be gapfilled are: 
```
