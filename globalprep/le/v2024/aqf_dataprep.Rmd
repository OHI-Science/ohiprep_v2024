...
```{r}
aqf_plot <- plot_ly(data = ornamental_revenue,
                    x = ~year,
                    y = ~value,
                    color = ~country,
                    type = "scatter",
...
top_10_revenue_yearly <- ornamental_revenue %>% 
  ungroup() %>% 
  group_by(year) %>% 
...
top_5_revenue_yearly <- ornamental_revenue %>% 
  ungroup() %>% 
  group_by(year) %>% 
  slice_max(value, n = 5) %>% 
  select(country, year, value) %>% 
  # COME BACK TO THIS -- tried 8 variations, not sure why it isn't working
  mutate(country = str_replace_all(country, pattern = " (Kingdom of the)", replacement = ""))
# str_remove_all(country, pattern = " (Kingdom of the)"))
...
Save intermediate data as csv in `int` folder
