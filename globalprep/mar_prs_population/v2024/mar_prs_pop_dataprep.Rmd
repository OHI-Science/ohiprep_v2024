...
Why reproject the ocean raster? It is a categorical raster defining zones, whereas the population raster is a continuous raster with highly variable cell values. As a result, changing the projection of the categorical raster would minimize errors propagating through calculations because it is 1. not directly modifying the data that we're interested in, and 2. categorical, indicating that the faster "nearest neighbor" method of resampling won't realistically change the underlying structure of the regions - only borders, which are typically less developed and less populated [And There Was Light:
Trade and the Development of Border Regions](https://people.unil.ch/mariusbrulhart/files/2023/06/borderlights.pdf). 
...
################### 
# Take a break here and check to make sure that the Mazu folder has both "yearly_change_YYYY_to_YYYY" files and "human_count_YYYY" files. Should be four and five, respectively
    ...
    terra::writeRaster(raster_initial, 
        ...
        for (i in 1:4) {
          ...
          if (yr <= end_year) { # Check that year is less than or equal to the end_year (should be)
            ...
            raster_current <- raster_initial + (i * change_raster) # calculate w/ yearly change * 
...
* NOTE: Creating these files and storing them in R will max out the tmp directory storage (limit 40GB) if other large objects are also loaded. It is highly recommended to use the yearly_interpolate_terra function but if that doesn't work then use this method but DONT run it all at once. Run one, check it to make sure it makes sense by runnning the following 
  
  * global(raster_2001, "sum", na.rm=TRUE)
  
  * Then write it out using writeRaster, and remove it from the environment using rm(object_name) before moving to the next one 
