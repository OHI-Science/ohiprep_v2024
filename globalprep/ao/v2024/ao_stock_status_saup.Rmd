...
# see whether there is a decrease or increase in bbmsy/mean_catch

# bbmsy
compare_bbmsy <- test_rgn3 %>% 
  left_join(test_rgn3_prev, by = c("year", "rgn_id")) %>% 
  filter(year == previous_assessment_latest_data_year) %>%
  mutate(diff = ((score - old_score)/old_score)*100) 
